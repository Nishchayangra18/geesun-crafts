...
function mapStockRpcError(message: string) {
  ...
  if (
    normalized.includes("could not find the function public.validate_and_decrement_stock") ||
    normalized.includes("could not find function public.validate_and_decrement_stock")
  ) {
    return "Checkout stock validation is not available yet. Please run the latest Supabase migrations and reload schema cache.";
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (stockError) {
      return NextResponse.json(
        { error: mapStockRpcError(stockError.message ?? "") },
        { status: 409 },
      );
    }
